...
const BASE_URL = 'https://vida-aide.purama.dev'
...
test.describe('P8 Cross-Browser Golden Path', () => {
  for (const browserName of ['chromium', 'webkit', 'firefox']) {
    test(`${browserName}: landing page → login → dashboard`, async ({ browser }) => {
      ...
      await expect(page).toHaveTitle(/PURAMA|Aide/)
      ...
      // no Stripe checkout test for vida-aide (aide search app, likely free tier)
